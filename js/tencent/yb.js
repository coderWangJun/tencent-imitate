...
let tencentRender = (function () {
    ...
    let queryData = (data, context) => {
        ...
        for (let i = 0; i < data.length; i++) {
            let {id, mark, mark_msg, hot, text, modidx, update, score, img_src, video_title} = data[i];
            _modidx = modidx;
    ...
    let collectControl = function () {
        ...
        for (let i = 0; i < $iControl.length; i++) {
            let item = $iControl[i];
...
(function (root, factory, plug) {
    factory(jQuery, plug);
        ...
        let obtain = function (e, _data) {
            ...
            let item = $target.parents('.figure_mark');
